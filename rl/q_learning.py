"""Tabular Q-Learning algorithm for the Tag game."""
...
class QLearning(BaseRLAlgorithm):
    ...
    def _bin_value(self, value: float, bins: int) -> int:
        """Map a scalar in [-1, 1] (or near range) into [0, bins-1]."""
        v = max(-1.0, min(1.0, value))
        return int((v + 1.0) * 0.5 * (bins - 1))

    def _state_key(self, obs: dict) -> tuple:
        """Discretize rich observation dict into a compact hashable state."""
    ...
    def learn(self, state: dict, action: int, reward: float,
        ...
        if done:
            ...
            next_max_q = max(self._q(next_state_key, na)
                             for na in range(self.ACTION_SPACE_SIZE))
