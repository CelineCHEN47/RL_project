...
class Menu:
    ...
    def _build_buttons(self):
        ...
        abtn_w = 130
        total_w = len(algo_names) * (abtn_w + 10) - 10
        start_x = cx - total_w // 2

        self.algo_buttons = []
        for i, name in enumerate(algo_names):
            x = start_x + i * (abtn_w + 10)
        ...
        # --- Start button ---
        self.start_button = Button(cx - 110, 540, 220, 50, "START GAME")
    ...
    def draw(self):
        ...
        self.screen.blit(subtitle, subtitle.get_rect(centerx=sw // 2, y=105))
        ...
        self.screen.blit(lbl, lbl.get_rect(centerx=sw // 2, y=158))
        for btn in self.mode_buttons:
            btn.draw(self.screen)

        # --- Section: Algorithm ---
        lbl = self.label_font.render("Algorithm", True, (180, 180, 200))
        self.screen.blit(lbl, lbl.get_rect(centerx=sw // 2, y=246))
        for btn in self.algo_buttons:
            btn.draw(self.screen)

        # --- Section: Train Mode ---
        lbl = self.label_font.render("Agent Behavior", True, (180, 180, 200))
        self.screen.blit(lbl, lbl.get_rect(centerx=sw // 2, y=358))
        for btn in self.train_buttons:
            btn.draw(self.screen)

        # Train mode warning if no model
        has_model = _model_exists(self.selected_algorithm)
        if not has_model:
            warn = self.hint_font.render(
                f"No trained model for {self.selected_algorithm}. "
                f"Train with: python train.py -a {self.selected_algorithm}",
                True, (200, 150, 80))
            self.screen.blit(warn, warn.get_rect(centerx=sw // 2, y=430))
        ...
        for i, hint in enumerate(hints):
            ...
            self.screen.blit(h, h.get_rect(centerx=sw // 2, y=620 + i * 20))

        ver = self.hint_font.render("v1.2", True, (70, 70, 90))
